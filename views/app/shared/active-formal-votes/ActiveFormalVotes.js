...
class ActiveFormalVotes extends Component {
  constructor(props) {
    super(props);
    this.state = {
      loading: false,
  ...
  startTracking() {
    ...
    this.$elem = document.getElementById("app-active-formal-votes-sectionBody");
  ...
  renderTooltip() {
    return (
      <Fragment>
        <p className="font-size-13">{`This area displays formal votes currently in progress. If you are a Voting Associate, you should review and vote.`}</p>
      </Fragment>
    );
  }
  ...
  renderQuorum(vote) {
    ...
    const checkQuorumMet =
      +vote.result_count / +vote.total_member >= +quorumRate / 100;

    return (
      <div className="font-size-14">
        <label>{vote.result_count}</label> /{" "}
        <label className="pr-2">{vote.total_member}</label>
  ...
  renderVotes() {
    ...
    votes.forEach((vote, index) => {
      ...
      if (vote.content_type == "grant") {
        if (settings.time_unit_formal && settings.time_formal) {
          if (settings.time_unit_formal == "min")
            minsAdd = parseInt(settings.time_formal);
          else if (settings.time_unit_formal == "hour")
            minsAdd = parseInt(settings.time_formal) * 60;
  ...
  render() {
    ...
    return (
      <Fade distance={"20px"} bottom duration={200} delay={700}>
        <section id="app-active-formal-votes-section" className={className}>
          <div className="app-infinite-search-wrap">
            {authUser.is_admin ? (
              <label>Active Formal Votes</label>
            ) : (
          ...
          <div className="infinite-content">
            <div className="infinite-contentInner">
              ...
              <div
                ...
                id="app-active-formal-votes-sectionBody"
              >
                {loading ? (
...
export default connect(mapStateToProps)(withRouter(ActiveFormalVotes));
