...
class SingleMilestoneVote extends Component {
  constructor(props) {
    super(props);
    this.state = {
      proposal: {},
  ...
  componentDidMount() {
    ...
    this.setState({ proposalId, voteId, loading: true }, () => {
      this.getProposal();
    });
  }
      ...
      getSingleProposal(
        ...
        (res) => {
          ...
          if (proposal && proposal.votes) {
            for (let i in proposal.votes) {
              if (proposal.votes[i].id == voteId) {
                vote = proposal.votes[i];
                break;
              }
            }
          }
      ...
      <div className="app-simple-section mb-3">
        <ul>
          <li>
            ...
            <b>{vote.type == "informal" ? "Informal" : "Formal"}</b>
          </li>
          <li>
...
export default connect(mapStateToProps)(withRouter(SingleMilestoneVote));
